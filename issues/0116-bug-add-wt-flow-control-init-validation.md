...
open issue 0117（`WtConfig` のフィールド private 化）が同じ注入経路を構造的に塞ぐため、本 issue の対応を 0117 に統合する。`WtConfig` のフィールドを private 化し、ビルダーに 2^60 上限検証を追加することで、手動構築による不正な初期値の注入を防ぐ。`WtFlowControl::new()` のシグネチャを Result 化する大規模な破壊的変更は行わない。
