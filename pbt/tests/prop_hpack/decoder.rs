...
proptest! {
    ...
    #[test]
    fn prop_bomb_is_exactly_bounded(
        value_len in 0usize..=300,
        ref_count in 0usize..=2000,
        max_size in 0usize..=65535,
        huffman_value in any::<bool>(),
    ) {
        ...
        if total > max_size {
            prop_assert!(
                result.is_err(),
                "総サイズ {} が上限 {} を超えるなら必ず Err",
                total,
                max_size
            );
        } else {
            let headers = result.expect("上限以下なら成功する");
            prop_assert_eq!(headers.len(), ref_count + 1);
            prop_assert_eq!(header_list_size(&headers), total);
    ...
    #[test]
    fn prop_unlimited_decodes_fully(
        value_len in 0usize..=200,
        ref_count in 0usize..=200,
        huffman_value in any::<bool>(),
    ) {
        let block = build_bomb(value_len, ref_count, huffman_value);
