...
fn stream_op() -> impl Strategy<Value = StreamOp> {
    prop_oneof![
        any::<bool>().prop_map(|end_stream| StreamOp::SendHeaders { end_stream }),
        any::<bool>().prop_map(|end_stream| StreamOp::RecvHeaders { end_stream }),
        any::<bool>().prop_map(|end_stream| StreamOp::SendData { end_stream }),
        any::<bool>().prop_map(|end_stream| StreamOp::RecvData { end_stream }),
        Just(StreamOp::SendRstStream),
        Just(StreamOp::RecvRstStream),
    ]
}
...
proptest! {
    ...
    /// 数学的意義: 吸収元の性質
    #[test]
    fn prop_closed_is_absorbing(
        ops_before in prop::collection::vec(stream_op(), 0..10),
        ops_after in prop::collection::vec(stream_op(), 1..10),
    ) {
        ...
        for op in ops_after {
            ...
            prop_assert_eq!(
                sm.state(),
                StreamState::Closed,
                "Closed state must be absorbing, but changed after {:?}",
                op
            );
        }
    }
    ...
    /// 数学的意義: 状態と述語の対応関係
    #[test]
    fn prop_can_send_recv_consistency(
        ops in prop::collection::vec(stream_op(), 0..20),
    ) {
        let mut sm = StateMachine::new();
        ...
        for op in ops {
            ...
            prop_assert_eq!(
                state.can_send(),
                expected_can_send,
                "can_send mismatch for state {:?}",
                state
            );

            // can_recv は Open または HalfClosedLocal でのみ true
            let expected_can_recv = matches!(
                state,
    ...
    /// 数学的意義: 状態遷移の閉包性
    #[test]
    fn prop_valid_transitions_only(
        ops in prop::collection::vec(stream_op(), 0..20),
    ) {
        let mut sm = StateMachine::new();
            ...
            match result {
                Ok(()) => {
                    ...
                    if state_before == StreamState::Closed {
                        prop_assert_eq!(state_after, StreamState::Closed);
                    }
                }
                Err(()) => {
                    ...
                    prop_assert_eq!(
                        state_before,
                        state_after,
                        "State changed after failed operation {:?}",
                        op
                    );
                }
            }
    ...
    /// 数学的意義: 終了状態への到達可能性
    #[test]
    fn prop_rst_stream_always_closes(
        ops in prop::collection::vec(stream_op(), 0..10),
        use_send in any::<bool>(),
    ) {
        let mut sm = StateMachine::new();
        ...
        prop_assert_eq!(
            sm.state(),
            StreamState::Closed,
            "RST_STREAM from {:?} should result in Closed",
            state_before
        );
    }
    ...
    /// 数学的意義: 初期状態からの有効な遷移
    #[test]
    fn prop_idle_to_open_via_headers(
        send in any::<bool>(),
    ) {
        let mut sm = StateMachine::new();
    ...
    /// 数学的意義: フラグと状態の整合性
    #[test]
    fn prop_end_stream_flags_consistency(
        ops in prop::collection::vec(stream_op(), 0..20),
    ) {
        let mut sm = StateMachine::new();
        ...
        for op in ops {
            ...
            prop_assert_eq!(
                sm.sent_end_stream(),
                expected_sent,
                "sent_end_stream mismatch for state {:?}",
                state
            );

            // received_end_stream は HalfClosedRemote または Closed でのみ true
            let expected_received = matches!(
                state,
            ...
            prop_assert_eq!(
                ...
                expected_received,
                "received_end_stream mismatch for state {:?}",
                state
            );
        }
    }
