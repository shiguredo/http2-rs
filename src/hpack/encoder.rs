...
impl Encoder {
    ...
    fn encode_indexed(&self, buf: &mut Vec<u8>, index: usize) {
        let mut temp = [0u8; 16];
        let len = integer::encode(&mut temp, index as u64, 7, 0x80).unwrap();
    ...
    fn encode_literal_indexed(&self, buf: &mut Vec<u8>, name_index: usize, value: &[u8]) {
        ...
        let mut temp = [0u8; 16];
        let len = integer::encode(&mut temp, name_index as u64, 6, 0x40).unwrap();
        buf.extend_from_slice(&temp[..len]);
    ...
    fn encode_literal_without_indexing_indexed(
        ...
        let mut temp = [0u8; 16];
        let len = integer::encode(&mut temp, name_index as u64, 4, 0x00).unwrap();
        buf.extend_from_slice(&temp[..len]);
    ...
    fn encode_never_indexed_with_name_index(
        ...
        let mut temp = [0u8; 16];
        let len = integer::encode(&mut temp, name_index as u64, 4, 0x10).unwrap();
        buf.extend_from_slice(&temp[..len]);
    ...
    fn encode_string(&self, buf: &mut Vec<u8>, data: &[u8]) {
        if self.use_huffman {
            ...
            if encoded.len() < data.len() {
                // Huffman 符号化の方が短い場合
                let mut temp = [0u8; 16];
                let len = integer::encode(&mut temp, encoded.len() as u64, 7, 0x80).unwrap();
                buf.extend_from_slice(&temp[..len]);
        ...
        // 平文でエンコード
        let mut temp = [0u8; 16];
        let len = integer::encode(&mut temp, data.len() as u64, 7, 0x00).unwrap();
        buf.extend_from_slice(&temp[..len]);
    ...
    pub fn encode_size_update(&self, buf: &mut Vec<u8>, new_size: usize) {
        let mut temp = [0u8; 16];
        let len = integer::encode(&mut temp, new_size as u64, 5, 0x20).unwrap();
        buf.extend_from_slice(&temp[..len]);
    }
}
