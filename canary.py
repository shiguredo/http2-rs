...
# cargo update shiguredo_http2 を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_http2'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_http2"], check=True)
        print("cargo update shiguredo_http2 executed")
