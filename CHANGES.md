...
- [FIX] Sans I/O 層で `:protocol=webtransport` 時の `:scheme=https` 検証を追加し、`:scheme=http` のリクエストを拒否する (draft-ietf-webtrans-http2-15 Section 3.2)
  - @voluntas
- [FIX] WT_STREAM_DATA_BLOCKED 受信時のストリーム状態チェックを仕様に合わせ、存在しないストリーム・受信不能なストリームへの受信で WT_STREAM_STATE_ERROR を返すようにする (draft-ietf-webtrans-http2-15 Section 6.9)
  - @voluntas
- [FIX] `WtFlowControl::add_recv_max` / `WtStream::update_recv_max` で varint 最大値 (2^62 - 1) 超過時にエラーを返し、後続の WT_MAX_DATA エンコードでの panic を防止する
  - @voluntas
- [FIX] `shiguredo_nghttp2::Session` の `unsafe impl Sync` を削除し、nghttp2 の非スレッドセーフな C 関数への同時アクセスによる UB を防止する
  - @voluntas
- [FIX] `shiguredo_nghttp2::Session::recv` の `ssize_t` 戻り値を `i32` に切り詰めていた問題を修正し、`i32::MAX` を超える入力でも正しく動作するようにする
  - @voluntas
