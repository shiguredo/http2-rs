...
struct StreamSendBuffer {
    ...
    /// ストリーム終了フラグ
    end_stream: bool,
    /// トレーラー送信フラグ（EOF 時に END_STREAM を付与しない）
    no_end_stream: bool,
}
...
pub struct Session {
    ...
    /// nghttp2 の最後のエラーメッセージ（error_callback2 で設定）
...
impl Session {
    ...
    /// 新しいセッションを作成
    fn new(role: SessionRole, options: Option<&SessionOptions>) -> Result<Self> {
        ...
        unsafe {
            ...
            nghttp2_sys::nghttp2_session_callbacks_set_on_frame_send_callback(
                callbacks,
                Some(on_frame_send_callback),
            );
            nghttp2_sys::nghttp2_session_callbacks_set_on_frame_not_send_callback(
                callbacks,
                Some(on_frame_not_send_callback),
            );
            nghttp2_sys::nghttp2_session_callbacks_set_on_invalid_frame_recv_callback(
                callbacks,
                Some(on_invalid_frame_recv_callback),
            );
            nghttp2_sys::nghttp2_session_callbacks_set_on_invalid_header_callback(
                callbacks,
                Some(on_invalid_header_callback),
            );
            nghttp2_sys::nghttp2_session_callbacks_set_error_callback2(
                callbacks,
                Some(on_error_callback2),
            );

            // セッションを作成
            let result = match (role, options) {
                (SessionRole::Client, Some(opts)) => nghttp2_sys::nghttp2_session_client_new2(
                    &mut session,
                    callbacks,
                    ptr::null_mut(),
                    opts.as_ptr(),
                ),
                (SessionRole::Client, None) => nghttp2_sys::nghttp2_session_client_new(
                    &mut session,
                    callbacks,
                    ptr::null_mut(),
                ),
                (SessionRole::Server, Some(opts)) => nghttp2_sys::nghttp2_session_server_new2(
                    &mut session,
                    callbacks,
                    ptr::null_mut(),
                    opts.as_ptr(),
                ),
                (SessionRole::Server, None) => nghttp2_sys::nghttp2_session_server_new(
                    &mut session,
                    callbacks,
                    ptr::null_mut(),
                ),
    ...
    /// トレーラーヘッダーを送信
    ///
    /// ストリームの DATA 送信後にトレーラーを送信する。
    /// data_source_read_callback で NGHTTP2_DATA_FLAG_NO_END_STREAM を設定し、
    /// その後このメソッドでトレーラーを送信する。
    pub fn submit_trailer(&mut self, stream_id: StreamId, headers: &[Header]) -> Result<()> {
        let nva = headers_to_nva(headers);
        unsafe {
            check_nghttp2(nghttp2_sys::nghttp2_submit_trailer(
                self.session,
                stream_id,
                nva.as_ptr(),
                nva.len(),
    ...
    /// 追加ヘッダーフレームを送信
    ///
    /// 既存ストリームに追加の HEADERS フレームを送信する (情報レスポンスなど)。
    pub fn submit_headers(
        ...
        let result = unsafe {
            nghttp2_sys::nghttp2_submit_headers(
                self.session,
                flags,
                stream_id,
                ptr::null(),
        ...
        if result < 0 {
            Err(Error::from_nghttp2(result))
        } else {
            Ok(())
        }
    }

    /// graceful shutdown 通知を送信
    ///
    /// last_stream_id に (1u31 << 31) - 1 を設定した GOAWAY を送信する。
    /// その後 `terminate_session()` で実際にセッションを終了する。
    pub fn submit_shutdown_notice(&mut self) -> Result<()> {
        unsafe { check_nghttp2(nghttp2_sys::nghttp2_submit_shutdown_notice(self.session)) }
    }
    ...
    /// GOAWAY を送信してセッションを終了する
    pub fn terminate_session(&mut self, error_code: ErrorCode) -> Result<()> {
        unsafe {
            check_nghttp2(nghttp2_sys::nghttp2_session_terminate_session(
                self.session,
                error_code.as_u32(),
            ))
        }
    ...
    /// リモートピアの SETTINGS 値を取得
    pub fn get_remote_settings(&self, id: SettingsId) -> u32 {
        unsafe {
            nghttp2_sys::nghttp2_session_get_remote_settings(
                self.session,
                id.as_i32() as nghttp2_sys::nghttp2_settings_id,
            )
        }
    }
    ...
    pub fn get_local_settings(&self, id: SettingsId) -> u32 {
        unsafe {
            nghttp2_sys::nghttp2_session_get_local_settings(
                self.session,
                id.as_i32() as nghttp2_sys::nghttp2_settings_id,
            )
        }
    ...
    pub fn get_outbound_queue_size(&self) -> usize {
        unsafe { nghttp2_sys::nghttp2_session_get_outbound_queue_size(self.session) }
    }

    /// 次に割り当てるストリーム ID を取得
    pub fn get_next_stream_id(&self) -> u32 {
        unsafe { nghttp2_sys::nghttp2_session_get_next_stream_id(self.session) }
    }

    /// 最後に処理したストリーム ID を取得
    pub fn get_last_proc_stream_id(&self) -> StreamId {
        unsafe { nghttp2_sys::nghttp2_session_get_last_proc_stream_id(self.session) }
    }
    ...
    /// コネクションレベルのリモートウィンドウサイズを取得
    pub fn get_remote_window_size(&self) -> i32 {
        unsafe { nghttp2_sys::nghttp2_session_get_remote_window_size(self.session) }
    }

    /// コネクションレベルのローカルウィンドウサイズを取得
    pub fn get_local_window_size(&self) -> i32 {
        unsafe { nghttp2_sys::nghttp2_session_get_local_window_size(self.session) }
    }

    /// ストリームのリモートウィンドウサイズを取得
    pub fn get_stream_remote_window_size(&self, stream_id: StreamId) -> Result<i32> {
        let result = unsafe {
            nghttp2_sys::nghttp2_session_get_stream_remote_window_size(self.session, stream_id)
        };
        if result < 0 {
    ...
    pub fn get_stream_local_window_size(&self, stream_id: StreamId) -> Result<i32> {
        let result = unsafe {
            nghttp2_sys::nghttp2_session_get_stream_local_window_size(self.session, stream_id)
        };
        if result < 0 {
            Err(Error::StreamNotFound(stream_id))
        } else {
    ...
    /// ローカルウィンドウサイズを設定
    ///
    /// `stream_id` が 0 の場合はコネクションレベルのウィンドウサイズを設定する。
    pub fn set_local_window_size(&mut self, stream_id: StreamId, window_size: i32) -> Result<()> {
        unsafe {
            check_nghttp2(nghttp2_sys::nghttp2_session_set_local_window_size(
                self.session,
                nghttp2_sys::nghttp2_flag_NGHTTP2_FLAG_NONE as u8,
                stream_id,
                window_size,
    ...
    /// 受信データを消費したことを通知
    ///
    /// コネクションとストリーム両方のウィンドウを更新する。
    /// `no_auto_window_update` オプション有効時に使用する。
    pub fn consume(&mut self, stream_id: StreamId, size: usize) -> Result<()> {
        unsafe {
            check_nghttp2(nghttp2_sys::nghttp2_session_consume(
                self.session,
                stream_id,
                size,
            ))
    ...
    /// コネクションレベルのウィンドウのみを更新
    pub fn consume_connection(&mut self, size: usize) -> Result<()> {
        unsafe {
            check_nghttp2(nghttp2_sys::nghttp2_session_consume_connection(
                self.session,
                size,
            ))
        }
    ...
    /// ストリームレベルのウィンドウのみを更新
    pub fn consume_stream(&mut self, stream_id: StreamId, size: usize) -> Result<()> {
        unsafe {
            check_nghttp2(nghttp2_sys::nghttp2_session_consume_stream(
                self.session,
                stream_id,
                size,
            ))
...
///
/// SAFETY: nghttp2_nv の name/value は *mut u8 だが、NO_COPY フラグなしの場合
/// nghttp2 は内部でデータをコピーするため、元のデータへの書き込みは発生しない。
/// 呼び出し元は nva の生存期間中 headers が有効であることを保証する必要がある。
        ...
        if send_buf.data.is_empty() {
            if send_buf.end_stream {
                ...
                *data_flags |= nghttp2_sys::nghttp2_data_flag_NGHTTP2_DATA_FLAG_EOF;
                if send_buf.no_end_stream {
                    // トレーラーモード: END_STREAM を付与しない
                    *data_flags |= nghttp2_sys::nghttp2_data_flag_NGHTTP2_DATA_FLAG_NO_END_STREAM;
                }
                return 0;
            }
...
extern "C" fn on_stream_close_callback(
    ...
    unsafe {
        if let Some(session) = get_session(user_data) {
            // ストリームに関連する全リソースをクリーンアップ
            session.send_buffers.remove(&stream_id);
            session.pending_headers.remove(&stream_id);
            session.pending_data.remove(&stream_id);
...
/// フレーム送信完了コールバック
extern "C" fn on_frame_send_callback(
    _session: *mut nghttp2_sys::nghttp2_session,
    frame: *const nghttp2_sys::nghttp2_frame,
    user_data: *mut c_void,
) -> libc::c_int {
    unsafe {
        if let Some(session) = get_session(user_data) {
            let frame = &*frame;
            if let Some(frame_type) = FrameType::from_u8(frame.hd.type_) {
                session.push_event(Http2Event::FrameSent {
                    stream_id: frame.hd.stream_id,
                    frame_type,
                });
            }
...
/// フレーム送信失敗コールバック
extern "C" fn on_frame_not_send_callback(
    _session: *mut nghttp2_sys::nghttp2_session,
    frame: *const nghttp2_sys::nghttp2_frame,
    ...
    unsafe {
        if let Some(session) = get_session(user_data) {
            let frame = &*frame;
            if let Some(frame_type) = FrameType::from_u8(frame.hd.type_) {
                session.push_event(Http2Event::FrameNotSent {
                    stream_id: frame.hd.stream_id,
                    frame_type,
                    lib_error_code,
                });
...
/// 不正フレーム受信コールバック
extern "C" fn on_invalid_frame_recv_callback(
    _session: *mut nghttp2_sys::nghttp2_session,
    frame: *const nghttp2_sys::nghttp2_frame,
    lib_error_code: libc::c_int,
    user_data: *mut c_void,
    ...
    unsafe {
        if let Some(session) = get_session(user_data) {
            let frame = &*frame;
            if let Some(frame_type) = FrameType::from_u8(frame.hd.type_) {
                session.push_event(Http2Event::InvalidFrameReceived {
                    stream_id: frame.hd.stream_id,
                    frame_type,
                    lib_error_code,
                });
...
/// 不正ヘッダー受信コールバック
extern "C" fn on_invalid_header_callback(
    _session: *mut nghttp2_sys::nghttp2_session,
    frame: *const nghttp2_sys::nghttp2_frame,
    name: *const u8,
    namelen: usize,
    ...
    unsafe {
        if let Some(session) = get_session(user_data) {
            ...
            session.push_event(Http2Event::InvalidHeaderReceived {
                stream_id: frame.hd.stream_id,
...
/// エラーコールバック (error_callback2)
///
/// nghttp2 がエラーを検出した際に呼ばれる。
/// エラーメッセージを Session に保存する。
extern "C" fn on_error_callback2(
    _session: *mut nghttp2_sys::nghttp2_session,
    _lib_error_code: libc::c_int,
    ...
    unsafe {
        if let Some(session) = get_session(user_data) {
            let msg_slice = std::slice::from_raw_parts(msg.cast::<u8>(), len);
            session.last_error_message = Some(String::from_utf8_lossy(msg_slice).into_owned());
        }
    }
    0
}
