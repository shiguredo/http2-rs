    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    /// データを送信
    ///
    /// 1 回の呼び出しで渡せる `data` は送信バッファの固定容量 (65535 bytes) までであり、
    /// それ以上は全量拒否されるため、呼び出し側で分割して渡すこと。送信待ちデータが
    /// 滞留している場合は、これより小さくても累積で容量を超えると拒否される。これは
    /// RFC 9113 Section 6.9 のフロー制御とは独立したローカルなバッファ上限である。
